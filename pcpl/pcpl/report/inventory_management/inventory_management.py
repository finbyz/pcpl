...
def execute(filters={'Company':'Prince Care Pharma Pvt. Ltd.'}):
	is_reposting_item_valuation_in_progress()
	...
	for bin in bin_list:
		...
		if item:
			parent_item_group = frappe.db.get_value('Item Group' , item.item_group ,  'parent_item_group')
		if not item:
		...
		data.append(
			[
				...
				re_order_qty,
				shortage_qty,
				bin.planned_qty + re_order_level - bin.actual_qty - bin.ordered_qty - bin.indented_qty
			]
		)
...
def get_columns():
	return [
		...
		{
			"label": _("Needs to raise Purchase Requisition "),
			"fieldname": "needs_to_raise_purchase_requisition",
			"fieldtype": "Float",
			"width": 100,
			"convertible": "qty",
		},
