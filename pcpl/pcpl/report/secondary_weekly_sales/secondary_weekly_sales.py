...
def execute(filters = {'year' : '2023-2024' , 'base_on':'Monthly' , 'group_by':'Sub Division' ,'select_month':['January',"February"]}):
...
def get_final_data(filters):
	...
	for row in territory_data:
		...
		if filters.get('group_by') == "Sub Division":
			conditions = ""
			conditions += "si.territory = '{}'".format(row.territory)
		gross_sa = 0
		return_cn = 0
		total_ns = 0
		total_ach = 0
		...
		final_data[(row.get('parent_territory'),row.get('zone') ,row.get('territory'))].update({'total_gs':gross_sa , 'total_cn':return_cn * (-1), 'total_ns':total_ns   ,'total_cnp':'{0:.2f}'.format((((return_cn * 100)/gross_sa)*(-1) if gross_sa else 0)) })
		if mo_target:
