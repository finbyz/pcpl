...
def get_item_map(item_code, include_uom):
	...
	items = frappe.db.sql("""
		...
		order by item.parent_item_group ASC, item.item_group ASC""".format(cf_field=cf_field, cf_join=cf_join, condition=condition),{"today": today(), "include_uom": include_uom}, as_dict=True)
