...
def execute(filters=None):
	...
	conditions = get_columns(filters, "Sales Invoice")
	data = get_data(filters, conditions)
	if filters.get('based_on') == 'Customer Group' and filters.get('group_by') == 'Customer':
		conditions["columns"].insert(0,"Parent Customer Group:Link/Customer Group")
	return conditions["columns"], data

def get_columns(filters, trans):
	if filters.get('period') == "Weekly" and (not filters.get('from_date') or not filters.get('to_date')):
		frappe.throw("Please Select From Date and To Date.")
	validate_filters(filters)

	# get conditions for based_on filter cond
	based_on_details = based_wise_columns_query(filters.get("based_on"), trans,filters)
...
def get_data(filters, conditions):
	...
	if filters.get('period') != "Weekly":
		...
		year_end_date = getdate(filters.get('end_date'))
	...
	if filters.get("group_by"):
		...
		grp_cond = ""
		if filters.get("parent_customer_group"):
			lft, rgt = frappe.db.get_value("Customer Group", filters["parent_customer_group"], ["lft", "rgt"])
			grp_cond = f"and t1.customer_group = t3.name and t3.parent_customer_group in (select name from `tabCustomer Group` where lft>={lft} and rgt<={rgt} and docstatus<2)"
			conditions["addl_tables"] = ",`tabCustomer Group` t3"
		...
		data1 = frappe.db.sql(
			""" select %s from `tab%s` t1, `tab%s Item` t2 %s
					where t2.parent = t1.name %s and t1.company = %s and %s between %s and %s and
					t1.docstatus = 1 %s %s
					group by %s
				"""
			% (
			...
			for i in range(len(row)):
				...
				row1 = frappe.db.sql(
					""" select %s , %s from `tab%s` t1, `tab%s Item` t2 %s
							where t2.parent = t1.name %s and t1.company = %s and %s between %s and %s
...
def period_wise_columns_query(filters, trans):
	query_details = ""
	pwc = []
	year_start_date, year_end_date= frappe.db.get_value("Fiscal Year", filters.get("fiscal_year"), ["year_start_date", "year_end_date"])
	if filters.get('period') == "Weekly":
		year_start_date = filters.get('from_date')
		year_end_date = filters.get('to_date')
	bet_dates = get_period_date_ranges(filters.get("period"), filters.get("fiscal_year"), year_start_date, year_end_date)
	...
	if filters.get("period") != "Yearly":
		week = 1
		for dt in bet_dates:
			get_period_wise_columns(dt, filters.get("period"), pwc, week)
...
def get_period_wise_columns(bet_dates, period, pwc, week):
	if period == "Monthly":
		pwc += [
		...
		pwc += [
			_(str(bet_dates[0].strftime("%d-%m-%Y")) + " TO " + str(bet_dates[1].strftime("%d-%m-%Y"))) + " (" + _("Qty") + "):Float:200",
			_(str(bet_dates[0].strftime("%d-%m-%Y")) + " TO " + str(bet_dates[1].strftime("%d-%m-%Y"))) + " (" + _("Amt") + "):Currency:200",
		]
	else:
		pwc += [
...
def get_period_wise_query(bet_dates, trans_date, query_details):
	query_details += """SUM(IF(t1.%(trans_date)s BETWEEN '%(sd)s' AND '%(ed)s', t2.stock_qty, NULL)),
					SUM(IF(t1.%(trans_date)s BETWEEN '%(sd)s' AND '%(ed)s', (t2.price_list_rate * t2.stock_qty), NULL)),
				""" % {
...
def get_period_date_ranges(period, fiscal_year=None, year_start_date=None, year_end_date=None):
	...
	total_week = weeks_between(getdate(year_start_date), getdate(year_end_date))
	...
	if period != "Weekly":
		...
		for i in range(1, total_week + 1, 1):
			...
			period_end_date = getdate(year_start_date) + relativedelta(weeks=1, days=-1)
			if period_end_date > getdate(year_end_date):
				period_end_date = getdate(year_end_date)
			period_date_ranges.append([year_start_date, period_end_date])
			year_start_date = period_end_date + relativedelta(days=1)
			if period_end_date == year_end_date:
				breaks
...
def based_wise_columns_query(based_on, trans,filters):
	based_on_details = {}
...
def get_data(filters, conditions):
	...
	if filters.get("group_by"):
		...
		if filters.get('based_on') == 'Customer Group' and filters.get('group_by') == 'Customer':
			for row in data:
				if row[0] != "" or row[0] != None or row[0]:
					pcg = frappe.db.get_value("Customer Group",row[0],'parent_customer_group')
					row.insert(0,pcg)					
	else:
		data = frappe.db.sql(
			""" select %s from `tab%s` t1, `tab%s Item` t2 %s
...
def based_wise_columns_query(based_on, trans,filters):
	...
	if based_on == "Item":
		...
		# 	cols = ["Parent Customer Group:Link/Customer Group"]
		based_on_details["based_on_cols"] = ["Customer Group:Link/Customer Group"]
		based_on_details["based_on_select"] = "t1.customer_group,"
		based_on_details["based_on_group_by"] = "t1.customer_group"
		based_on_details["addl_tables"] = ""
