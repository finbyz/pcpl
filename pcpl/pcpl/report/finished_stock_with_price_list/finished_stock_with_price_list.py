...
def execute(filters=None):
	...
	for (company, item, warehouse) in sorted(iwb_map):
		if item_map.get(item):
			...
			# if not report_data.get("price_list_rate",0):
			# 	report_data["price_list_rate"]=0
			report_data['bal_val_on_pl']=flt(report_data.get("bal_qty",0) or 0)*flt(report_data.get("price_list_rate",0) or 0)
...
def get_columns(filters):
	...
	columns = [
		...
		{
			"label": _("Balance Value on Price list"),
			"fieldname": "bal_val_on_pl",
			"fieldtype": "Currency",
			"width": 300,
			"options": "currency",
		},
...
def get_stock_ledger_entries(filters, items):
	...
	item_group_range=frappe.db.get_all("Item Group",{"name":['in',["10000 Finish goods (F)","B Non Conversion Items"]]},['lft','rgt'])
	if item_group_range:
		conditions+=" and "
		range_cond=""
		for each in item_group_range:
			if range_cond:
				range_cond+=" or "
			range_cond += f"(ig.lft> {each.lft} and ig.rgt< {each.rgt} )"
	...
	return frappe.db.sql(
		...
		left join `tabItem` as i on i.item_code = sle.item_code
...
def get_item_details(items, sle, filters):
	...
	res = frappe.db.sql(
		"""
		select
			item.name, item.item_name, item.description, item.item_group, item.brand,ip.price_list_rate, item.stock_uom %s
		from
			`tabItem` item
			%s
		left join `tabItem Price` as ip on ip.item_code=item.item_code and ip.price_list= '%s'
		where
			item.name in (%s)
	"""
		% (cf_field, cf_join,filters.get("price_list"), ",".join(["%s"] * len(items))),
		items,
		as_dict=1,
	)
