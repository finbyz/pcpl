...
def get_items(filters):
	...
	items = frappe.db.sql("""
		select
			...
			sum(`tabPurchase Invoice Item`.stock_qty) as stock_qty,
			sum(`tabPurchase Invoice Item`.base_net_amount) as base_net_amount,
			sum(`tabPurchase Invoice Item`.base_price_list_rate) as base_price_list_rate,
			`tabPurchase Invoice Item`.parent, `tabPurchase Invoice Item`.item_code,
			`tabGST HSN Code`.description
		from `tabPurchase Invoice`, `tabPurchase Invoice Item`, `tabGST HSN Code`
		where `tabPurchase Invoice`.name = `tabPurchase Invoice Item`.parent
			and `tabPurchase Invoice`.docstatus = 1
			and `tabPurchase Invoice Item`.gst_hsn_code is not NULL
			and `tabPurchase Invoice Item`.gst_hsn_code = `tabGST HSN Code`.name %s %s
		group by
			`tabPurchase Invoice Item`.parent, `tabPurchase Invoice Item`.item_code
...
def get_tax_conditions(filters):
	conditions = " and `tabPurchase Taxes and Charges`.account_head LIKE '%%GST%%'"
...
def get_tax_accounts(
	...
	tax_query = frappe.db.sql(
		"""
		select
			`tabPurchase Taxes and Charges`.item_wise_tax_detail, `tabPurchase Taxes and Charges`.parent, `tabPurchase Taxes and Charges`.name as tax_name
		from
			`tabPurchase Invoice`,
			`tabPurchase Taxes and Charges`
		where
			`tabPurchase Taxes and Charges`.parent = `tabPurchase Invoice`.name
			and `tabPurchase Taxes and Charges`.tax_amount != 0
			and `tabPurchase Invoice`.docstatus = 1 %s %s
		"""
