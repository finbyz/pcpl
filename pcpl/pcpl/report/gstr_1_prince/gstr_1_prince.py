			...
			for inv, items_based_on_rate in self.items_based_on_tax_rate.items():
				...
				for rate, items in items_based_on_rate.items():
					...
					if self.filters.get("type_of_business") in ("CDNR-REG", "CDNR-UNREG"):
						row.update({"reverse_charge":"Y" if invoice_details.posting_date <= date(2017, 7, 1) else "N"})
						row.update({"document_type":"C" if invoice_details.is_return else "D"})
	...
	def get_row_data_for_invoice(self, invoice, invoice_details, tax_rate, items):
		...
		for fieldname in self.invoice_fields:
			if self.filters.get("type_of_business") in ("CDNR-REG", "CDNR-UNREG") and fieldname == "invoice_value":
				...
				row_dict.update({"invoice_value":abs(invoice_details.base_rounded_total) or abs(invoice_details.base_grand_total)})
			elif fieldname == "invoice_value":
				row.append(invoice_details.base_rounded_total or invoice_details.base_grand_total)
				row_dict.update({"invoice_value":invoice_details.base_rounded_total or invoice_details.base_grand_total})
			elif fieldname in ('posting_date', 'shipping_bill_date'):
				row.append(formatdate(invoice_details.get(fieldname), 'dd-MMM-YY'))
				row_dict.update({fieldname:formatdate(invoice_details.get(fieldname), 'dd-MMM-YY')})
				...
				row_dict.update({fieldname:export_type})
			elif fieldname == 'cess_amount':
				row.append(flt(self.invoice_cess.get(invoice), 2))
				row_dict.update({'cess_amount':flt(self.invoice_cess.get(invoice), 2)})
			else:
				row.append(invoice_details.get(fieldname))
				row_dict.update({fieldname:invoice_details.get(fieldname)})
		taxable_value = 0
		...
		row_dict.update({'taxable_value':flt(taxable_value , 2)})
		row_dict.update({'rate':flt(tax_rate , 2)})
		# for column in self.other_columns:
		# 	if column.get('fieldname') == 'cess_amount':
		# 		row.append(flt(self.invoice_cess.get(invoice), 2))
		# 		row_dict.update({'cess_amount':flt(self.invoice_cess.get(invoice), 2)})
		return row_dict, taxable_value 
